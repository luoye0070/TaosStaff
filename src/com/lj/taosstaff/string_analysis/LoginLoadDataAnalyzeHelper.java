package com.lj.taosstaff.string_analysis;
		...
		try {
			...
			while (jrd.hasNext()) {
				...
				if (tagName.equals("recode")) {// 返回的结果码
					...
					jrd.endObject();
				} else if (tagName.equals("staffInfo")) {// 返回的用户信息
					if (this.recode == 0) {
						...
						while (jrd.hasNext()) {
							...
							} else if (tagName.equals("loginName")) {// 解析用户名
								try {
									String str = jrd.nextString();
									System.out.println(str);
							...
							} else if (tagName.equals("restaurantId")) {// 解析用户名
								try {
									String str = jrd.nextString();
									System.out.println(str);
						...
						try {
							...
							while (jrd.hasNext()) {
								...
								while (jrd.hasNext()) {
									tagName = jrd.nextName();
									System.out.print(tagName + ":");
									if (tagName.equals("positionType")) {// 解析商品列表
										try {
											int i_i = jrd.nextInt();
											System.out.println(i_i);
											this.userInfo.positionTypeList
													.add(i_i);
										} catch (IllegalStateException e) {
											e.printStackTrace();
											jrd.nextNull();
